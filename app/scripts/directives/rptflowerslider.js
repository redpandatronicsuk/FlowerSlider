...
/**
 * @ngdoc directive
 * @name flowerSliderApp.directive:rptFlowerSlider
 * @description
 * # rptFlowerSlider
 */
 angular.module('flowerSliderApp')
 .directive('rptFlowerSlider', function () {
 	return {
      //template: '<svg></svg>',
      restrict: 'E',
      scope: { numSliders: '=',
      ...
      link: function postLink(scope, element, attrs) {
      	var sliderPrefix;
      	if (scope.sliderPrefix) {
      		sliderPrefix = scope.sliderPrefix;
      	} else {
      		sliderPrefix = 'sl';
      	}
      	...
      	if (scope.stepSliders) {
      		scope.stepSliders.forEach(function(stepSlider) {
      			stepSliders.push(sliderPrefix + stepSlider.toString());
      		});
      	}
      	...
      	var stepSliders = d3.set(stepSliders);
      	var svg, drag, defs, numFormatter, sliders = [], sliderValues = {};
      	init();
	...
	sliderAngles.forEach(function(rotAngle,i) {
		...
		makeGradientDef(i);
		tmpSlider.append("path").attr('fill','url(#' + sliderPrefix + i + 'gr)').attr('d','M-10,16l175.51,39.684v-0.445 c17.498-32.229,17.498-81.838,0-110.923L-10-16')
		.attr('stroke-linecap', "null").attr('stroke-linejoin', "null").attr('stroke-dasharray' ,"null");

		// Make group for draggable slider:
		var sldG = tmpSlider.append("g").call(drag).attr('transform','scale(0.8)').attr('id', sliderPrefix + i);
		...
		// Mouth:
		tmpSmile.append('path').attr('id', sliderPrefix + i + 'sml').attr('fill','none').attr('stroke','black').attr('d', 'M 17 42 q 16 0 30 0').attr('stroke-width', 5);
		// Left eye:
		...
		// Percentage text:
		tmpSlider.append("text").attr("id", sliderPrefix + i + "tx").text( function(d) { return d.x; }).attr("x", function(d) { return d.x; }).attr("y", function(d) { return d.y+5; }).attr('text-anchor','middle').attr('alignment-baseline', 'middle').attr('visibility','hidden');
...
function moveSlider(x, that, showPerc) {
	var mCx = x - 50;
	var mS = (x+25) * .01;
	d3.select(that)
	.attr("opacity", 0.6)
...
function makeGradientDef(id) {
	...
	.attr('id', 'sl' + id + 'gr').call(
		function(gradient) {
			gradient.append('svg:stop').attr('id', 'sl' + id + 'gr1').attr('offset', '0.5').attr('stop-color', '#0099ff').attr('stop-opacity',1);
...
function initRadialGradient() {
	...
	svg.append('circle').attr('id','rgCirc').attr('r', 300).attr('cx', 300).attr('cy', 300).attr('stroke-linecap', 'null').attr('stroke-linejoin', 'null').attr('stroke-dasharray', 'null').attr('stroke-width', 0).attr('fill','url(#rdlGrdnt)');
}
